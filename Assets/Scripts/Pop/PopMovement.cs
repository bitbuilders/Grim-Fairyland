...
public class PopMovement : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (horz == 0.0f) m_velocity.x -= Mathf.Sign(m_velocity.x) * m_acceleration * Time.deltaTime;
        if (vert == 0.0f) m_velocity.z -= Mathf.Sign(m_velocity.z) * m_acceleration * Time.deltaTime;
